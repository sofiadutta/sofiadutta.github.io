;(function () {
	...
	var enhanceContactEmail = function() {
		...
		var obfuscatedText = p.textContent || p.innerText;
		var email = obfuscatedText.replace(/ DOT /g, '.').replace(/ AT /g, '@').replace(/ /g, '');

		var mailtoLink = document.createElement('a');
		mailtoLink.href = 'mailto:' + encodeURIComponent(email).replace(/%40/g, '@');
		mailtoLink.textContent = email;

		p.textContent = '';
		p.appendChild(mailtoLink);
