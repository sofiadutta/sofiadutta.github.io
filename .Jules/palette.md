...
## 2024-05-23 - [Accessible Copy Buttons for Obfuscated Emails]
**Learning:** Adding a "Copy" button for obfuscated email addresses (e.g., `user AT domain DOT com`) significantly improves UX, but developers often make the mistake of storing the raw, un-obfuscated email address in a `data-*` attribute on the button itself. This defeats the purpose of the obfuscation entirely, as spambots can easily scrape data attributes.
**Action:** When creating a copy button for obfuscated text, always read the obfuscated text dynamically from the DOM and de-obfuscate it *client-side* inside the click event listener (e.g., using regex replacements) before writing to the clipboard.
