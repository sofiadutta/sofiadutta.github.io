...
## 2025-02-19 - Iframe Sandbox Fix
**Vulnerability:** Embedded third-party content (YouTube videos) without `sandbox` attributes.
**Learning:** Without `sandbox` attributes, embedded third-party content can execute potentially malicious code and perform unauthorized top-level navigation, leading to XSS breakout risks.
**Prevention:** Always implement defense-in-depth security by adding restrictive `sandbox` attributes (e.g., `allow-scripts allow-same-origin allow-presentation allow-popups`) to `iframe` elements.
