...
## 2025-02-19 - Secure Iframe Embedding
**Vulnerability:** Embedded third-party content (YouTube videos) using `iframe` lacked security restrictions, posing potential XSS breakouts and unauthorized top-level navigation risks.
**Learning:** By default, `iframe`s have full privileges. Adding the `sandbox` attribute restricts the embedded content.
**Prevention:** Always use restrictive `sandbox` attributes (e.g., `allow-scripts allow-same-origin allow-presentation allow-popups`) on `iframe`s loading third-party content for defense-in-depth security.
