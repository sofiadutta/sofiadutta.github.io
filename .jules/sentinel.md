...
## 2025-02-19 - Missing iframe sandboxing on YouTube embeds
**Vulnerability:** Third-party iframes without the `sandbox` attribute.
**Learning:** External content embedded via iframe can potentially run malicious scripts or break out of the frame. It's important to restrict what the iframe can do.
**Prevention:** Always add the `sandbox` attribute with only the necessary permissions (e.g., `allow-scripts allow-same-origin allow-presentation allow-popups`) when embedding third-party content like YouTube videos.
