...
## 2026-03-18 - Defense-in-depth for Third-Party Iframes
**Vulnerability:** Third-party iframes lacking sandbox attributes allow potential XSS breakouts or unauthorized top-level navigation.
**Learning:** Even trusted third-party content (like YouTube videos) should be embedded securely. Without a restrictive sandbox, a compromised third party could take over the parent page.
**Prevention:** Always enforce defense-in-depth security by adding restrictive `sandbox` attributes (e.g., `allow-scripts allow-same-origin allow-presentation allow-popups`) when embedding third-party content via `iframe`.
