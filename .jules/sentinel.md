...
## 2025-05-22 - Restrictive Sandbox Attributes on Third-Party Iframes
**Vulnerability:** Missing `sandbox` attributes on third-party `iframe` elements (e.g., YouTube embeds).
**Learning:** Third-party iframes without a restrictive `sandbox` attribute present a defense-in-depth security risk, as compromised third-party content could attempt a cross-site scripting (XSS) breakout.
**Prevention:** Always include restrictive `sandbox` attributes (e.g., `allow-scripts allow-same-origin allow-presentation allow-popups`) on `iframe` tags.
