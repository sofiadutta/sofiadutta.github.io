...
## 2025-02-20 - Defense-in-Depth for Third-Party Iframes
**Vulnerability:** Unrestricted third-party iframes (YouTube) allowing potential XSS breakouts or unauthorized top-level navigation.
**Learning:** Even with a strong CSP, embedded iframes from external sources can execute scripts that might navigate the top-level browsing context or show popups if compromised. Defense-in-depth requires sandboxing them.
**Prevention:** Always apply restrictive `sandbox` attributes (e.g., `allow-scripts allow-same-origin allow-presentation allow-popups`) to `<iframe>` elements loading external content.
