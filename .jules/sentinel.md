...
## 2025-02-19 - Secure Third-Party Iframes
**Vulnerability:** Third-party `iframe` elements lacked restrictive `sandbox` attributes, creating a defense-in-depth risk for XSS breakout.
**Learning:** Even trusted embeds (like YouTube) should be sandboxed to restrict their capabilities within the host application.
**Prevention:** Always include `sandbox="allow-scripts allow-same-origin allow-presentation allow-popups"` (or more restrictive rules) on third-party `iframe` embeds.
