...
## 2025-02-23 - Defense-in-depth for iframes
**Vulnerability:** Embedded third-party iframes (e.g., YouTube videos) lacking sandbox attributes can potentially execute unauthorized top-level navigation or XSS breakouts.
**Learning:** Providing an explicit `sandbox` attribute restricts the permissions of third-party content, enforcing defense-in-depth security.
**Prevention:** Always add `sandbox="allow-scripts allow-same-origin allow-presentation allow-popups"` (or stricter) to third-party iframes.
