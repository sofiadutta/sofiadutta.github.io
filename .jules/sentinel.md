...
## 2025-02-19 - Strict Containment for Third-Party Embeds
**Vulnerability:** Unrestricted third-party iframes missing the `sandbox` attribute, exposing the application to breakout risks if the embed is compromised.
**Learning:** The project relies on external multimedia embeds (e.g., YouTube). Applying a strict `sandbox` policy is a critical reusable security pattern for all external iframes in this app architecture to prevent XSS.
**Prevention:** Always apply `sandbox="allow-scripts allow-same-origin allow-presentation allow-popups"` to any `iframe` elements added to the codebase.
