...
## 2025-04-07 - Iframe Defense in Depth
**Vulnerability:** Third-party iframes (e.g., YouTube embeds) lacking the `sandbox` attribute.
**Learning:** Even with CSP `frame-src` restricting allowed origins, iframes without `sandbox` attributes still carry a risk of XSS breakouts or top-level navigation hijacking if the third-party origin is compromised.
**Prevention:** Always include restrictive `sandbox` attributes (`allow-scripts allow-same-origin allow-presentation allow-popups`) on all third-party `iframe` elements to enforce least privilege.
