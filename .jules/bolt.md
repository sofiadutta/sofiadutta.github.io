...
## 2025-01-08 - Throttle Scroll Events
**Learning:** Unthrottled jQuery scroll listeners (like `$(window).scroll`) can cause layout thrashing and block the main thread.
**Action:** Replace unthrottled jQuery scroll listeners with native `window.addEventListener('scroll', ...)` and throttle DOM updates (using `classList`, `querySelector`) via `requestAnimationFrame` and a boolean lock to optimize scrolling performance.
