...
## 2025-01-08 - Scroll Event Performance Anti-pattern
**Learning:** The unthrottled `$(window).scroll()` jQuery event listener was blocking the main thread and causing layout thrashing due to continuous firing on scroll.
**Action:** Replace unthrottled jQuery scroll events with native `window.addEventListener('scroll', ...)` with `{ passive: true }`, and throttle DOM updates (via `classList` and `querySelector`) using `requestAnimationFrame` and a boolean lock.
