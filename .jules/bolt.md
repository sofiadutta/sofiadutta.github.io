...
## 2026-03-10 - Unthrottled Scroll Handlers
**Learning:** The codebase utilizes unthrottled jQuery `$(window).scroll` listeners for offcanvas menu dismissal, leading to main-thread jank during heavy scrolling.
**Action:** Use `window.requestAnimationFrame` with a boolean ticking flag to optimize heavy scroll event listeners instead of basic debouncing to guarantee immediate UI updates without thread blocking.
