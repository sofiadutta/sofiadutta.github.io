...
**Learning:** Unthrottled scroll listeners, specifically those using jQuery like `$(window).scroll`, execute multiple times per frame and block the main thread, causing significant layout thrashing.
**Action:** Replace unthrottled jQuery scroll listeners with native `window.addEventListener('scroll', ...)` and throttle DOM queries (e.g., `classList`) using `requestAnimationFrame` and a boolean lock.
