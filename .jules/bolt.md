...
**Learning:** Attaching continuous event listeners to window.scroll for visual state updates (like closing mobile offcanvas menus) can cause layout thrashing. Debouncing the scroll event is crucial.
**Action:** When debouncing scroll events that trigger immediate visual updates, always use the `immediate=true` configuration for the debounce function so the interaction responds instantly, whilst throttling subsequent events.
