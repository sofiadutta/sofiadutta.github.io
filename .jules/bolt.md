...
**Learning:** The codebase relies on unthrottled `$(window).scroll()` for scroll behaviors, which causes severe layout thrashing and main thread blocking.
**Action:** Always replace unthrottled `$(window).scroll()` handlers with native `window.addEventListener('scroll', ...)` and throttle DOM updates using a boolean lock and `requestAnimationFrame()`.
