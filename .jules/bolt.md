...
**Learning:** The codebase relies on unthrottled `$(window).scroll()` event handlers in `js/main.js` (e.g., for `mobileMenuOutsideClick`), which can cause significant layout thrashing and main thread blocking.
**Action:** Replace `$(window).scroll(...)` with native `window.addEventListener('scroll', ...)` paired with a boolean lock and `window.requestAnimationFrame()` to throttle DOM queries (using native `querySelector`) and updates.
