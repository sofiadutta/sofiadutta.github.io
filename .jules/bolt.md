...
## 2025-01-09 - Debouncing Scroll Events for UX
**Learning:** Un-debounced scroll event listeners can severely impact performance by executing continuously on the main thread during scrolling.
**Action:** Wrap scroll and resize event listeners in a debounce or throttle function. For immediate visual UI changes (like closing an offcanvas menu), use a leading-edge debounce (`immediate=true`) to avoid noticeable UX lag.
