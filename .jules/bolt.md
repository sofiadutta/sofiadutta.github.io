...
**Learning:** Using unthrottled `$(window).scroll()` causes layout thrashing and main thread blocking due to frequent DOM manipulations and jQuery wrapper overhead.
**Action:** Always replace unthrottled jQuery scroll listeners with native `window.addEventListener('scroll', ...)` and throttle DOM updates using `requestAnimationFrame` and a boolean lock.
