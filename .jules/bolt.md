...
**Learning:** This codebase uses unthrottled jQuery listeners (e.g., `$(window).scroll()`) that repeatedly instantiate jQuery objects like `$('body')` on every event tick, causing unnecessary garbage collection overhead and main thread blocking.
**Action:** Always replace unthrottled jQuery scroll listeners with native `window.addEventListener('scroll', ...)` using a boolean lock, `requestAnimationFrame`, and `{ passive: true }`. Replace internal jQuery queries with native DOM API (e.g., `document.body.classList`) to minimize overhead.
