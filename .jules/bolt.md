...
**Learning:** Unthrottled jQuery scroll listeners (e.g., `$(window).scroll`) cause severe layout thrashing and main thread blocking in this codebase's architecture.
**Action:** Always replace them with native `window.addEventListener('scroll', ...)` using `{ passive: true }` and throttle DOM updates via `requestAnimationFrame` and a boolean lock.
